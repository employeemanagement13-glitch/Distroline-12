-- ============================================================================
-- MDOS v26 — Sale / Purchase Summary & Return Amt Input Fix
--
-- Fix 1: sell_in_arrived_lines extension — adds returned_amt column for per-PO
--        return amount inputs (specifically for RGB products).
--
-- Fix 2: Grouping by normalized product_name across ALL subqueries so each product
--        name appears EXACTLY ONCE on Sale / Purchase Summary (no duplicate / dummy rows).
--
-- Fix 3: Sale Amt = Sale Qty * p.sale_rate.
--
-- Fix 4: Ret Amt logic:
--        - is_rgb = TRUE  → SUM(sal.returned_amt) entered in per-PO Return Amt input
--        - is_rgb = FALSE & is_returnable = TRUE → SUM(sal.returned_qty * p.purchase_rate)
--
-- Fix 5: S.Ret Qty/Amt for is_returnable = FALSE products only.
--        S.Ret Amt = S.Ret Qty * p.sale_rate.
-- ============================================================================

-- 1. ADD returned_amt to sell_in_arrived_lines
ALTER TABLE sell_in_arrived_lines
  ADD COLUMN IF NOT EXISTS returned_amt NUMERIC(12,2) NOT NULL DEFAULT 0;


-- 2. RECREATE sale_purchase_summary VIEW
...
-- 3. RECREATE RPC get_sale_purchase_filtered
