-- ============================================================================
-- MDOS v29 — Sale / Purchase Summary RGB & Returnable Exclusion Fix
-- ============================================================================
...
WITH pd AS (
  SELECT DISTINCT ON (p.tenant_id, LOWER(TRIM(p.product_name)))
    ...
    COALESCE(
      ...
      p.is_returnable,
      CASE
        WHEN LOWER(TRIM(p.product_name)) LIKE '%rgb%'
...
ret AS (
  SELECT
    ...
    SUM(CASE
      ...
      ELSE COALESCE(sal.returned_qty, 0) * COALESCE(pd.purchase_rate, 0)
